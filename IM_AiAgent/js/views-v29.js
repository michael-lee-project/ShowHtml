  ...
  'chat-list': () => {
    ...
    return `
      <div class="phone-view active chat-list-view ai-marketing-msg" data-view="chat-list">
        ...
        <div class="msg-section-head scene-head">
          <span>AI AUTO TALK SCENES</span>
          <em>核心场景演示</em>
        </div>
        <div class="ai-scene-strip">
...
function getAiScenarioDemo(id) {
  const demos = {
    silent: {
      ...
      result:{main:'客户重新进入可跟进队列', items:['客户意向：+6','下一步：等待客户选择 A/B','人工建议：无需接管']},
      steps:[
        {type:'customer', text:'我先考虑一下，晚点再说。'},
        {type:'thinking', lines:['检测状态：3 天未回复','读取历史：客户曾咨询体验装价格','匹配策略：轻提醒 + 低压力选择题']},
        {type:'ai', text:'小雅，前几天你问的体验装我帮你保留了。你不用现在决定，我先把两种方式发你。'},
        {type:'customer', text:'哪两种方式？'},
        {type:'thinking', lines:['客户重新响应','判断可继续沟通','生成 A/B 选择式回复']},
        {type:'ai', text:'A 是先体验流程，B 是直接看升级方案。你选一个，我按你的节奏详细说。'}
      ]
    },
    price: {
      ...
      result:{main:'客户从拒绝转为愿意先看体验装', items:['客户意向：+5','下一步：发送体验装链接','人工建议：无需接管']},
      steps:[
        {type:'customer', text:'感觉还是有点贵，我怕做不起来。'},
        {type:'thinking', lines:['识别异议：价格敏感','真实顾虑：投入后做不起来','匹配策略：先认同，再拆低门槛']},
        {type:'ai', text:'我理解，你不是觉得没价值，而是担心投入后做不起来。所以我不建议你一开始上高配版。'},
        {type:'customer', text:'那有没有风险低一点的方式？'},
        {type:'thinking', lines:['客户接受低门槛方案','推荐商品：体验装','回复目标：降低试错成本']},
        {type:'ai', text:'有，你可以先从体验装开始。确认适合后再升级，前面的金额还能抵扣。'}
      ]
    },
    close: {
      ...
      result:{main:'客户进入下单流程', items:['客户意向：+12','下一步：发送支付方式','人工建议：成交后跟进开通']},
      steps:[
        {type:'customer', text:'那我要怎么付款？'},
        {type:'thinking', lines:['识别意图：强成交信号','客户阶段：准备下单','下一步：发送支付方式 + 注意事项']},
        {type:'ai', text:'可以的，我现在发你下单方式。你先选体验装版本，下单后我会帮你开通新人流程。'},
        {type:'customer', text:'下单后第一步要做什么？'},
        {type:'thinking', lines:['客户已进入成交后预期管理','匹配 SOP：开通 + 新人任务','生成第一天行动清单']},
        {type:'ai', text:'第一天只做 3 件事：完善资料、熟悉话术、跟进 3 个意向客户。我会一步步提醒你。'}
      ]
    },
    complaint: {
      ...
      result:{main:'投诉风险被压低', items:['风险等级：高 → 中','下一步：同步物流和补偿','人工建议：需要关注']},
      steps:[
        {type:'customer', text:'你们这个发货太慢了，我有点不想要了。'},
        {type:'thinking', lines:['识别情绪：不满 / 可能退款','优先级：高','策略：先道歉 + 给明确处理时间']},
        {type:'ai', text:'真的不好意思，这次让你等久了。我先帮你查物流，如果今天没有更新，我会直接帮你申请补偿方案。'},
        {type:'customer', text:'那你别让我重复说一遍了。'},
        {type:'thinking', lines:['识别诉求：不想重复说明','动作：沉淀工单摘要','同步人工客服：完整上下文']},
        {type:'ai', text:'不会让你重复说明。我已经把订单、问题和诉求整理好，同步给人工客服继续处理。'}
      ]
    },
    repurchase: {
      ...
      result:{main:'复购机会已生成', items:['客户意向：+7','下一步：发送同款补货组合','人工建议：无需接管']},
      steps:[
        {type:'customer', text:'最近有老客户活动吗？'},
        {type:'thinking', lines:['读取订单：上次购买已 28 天','识别周期：接近复购窗口','匹配策略：提醒补货 + 老客户券']},
        {type:'ai', text:'你上次买的那套应该快用完了。我看你更喜欢基础款，这次可以直接补同款。'},
        {type:'customer', text:'有优惠券的话发我看看。'},
        {type:'thinking', lines:['客户接受优惠触达','匹配券包：老客户复购券','下一步：推送补货组合']},
        {type:'ai', text:'有一张老客户券今天可用。我把同款补货组合和券一起发你，适合直接补一轮。'}
      ]
    },
    'meeting-follow': {
      ...
      result:{main:'会后跟进已完成', items:['会议重点：3 条','下一步：确认方案版本','人工建议：15 分钟内跟进']},
      steps:[
        ...
        {type:'customer', text:'可以，先发我看看。'},
        {type:'thinking', lines:['生成会后摘要：交付时间 / 售后保障 / 团队使用','关联会议待办','准备方案确认路径']},
        {type:'ai', text:'第一是交付时间，第二是售后保障，第三是团队使用方式。你先看这版，没问题我再帮你排期。'}
      ]
    }
  };
  const picked = demos[id] || demos.silent;
  const steps = picked.steps || [];
  return {...picked, doneDelay:(steps.length * .72 + .9).toFixed(1)};
}
...
function renderAiDemoStep(step, idx, c, demo) {
  const delay = (idx * .72 + .18).toFixed(1);
